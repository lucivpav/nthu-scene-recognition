...
% spatial pyramid
function image_feats = get_pyramid_bags_of_sifts(image_paths)
...
%}
  ...
  for i=1:N
    ...
    [locations, features] = vl_dsift(img, 'step', 10, 'fast');
    ...
    for l=0:L
      ...
      h = compute_histogram(locations, index, size(img), vocab_size, l);
      h = h ./ w;
      feats = [feats h];
    end
    image_feats(i,:) = feats;
...
function features = compute_histogram(locations, feats, area, vocab_size, level)
  step = area ./ 1/(2^level);
  x = y = 0;
  features = [];
  while x <= size(area,2)
    while y <= size(area,1)
      % find all features within current block
      feats_in_block = [];
      for f=1:size(feats)
        if ( locations(1,f) > x &&
             locations(1,f) <= x+step(2) &&
             locations(2,f) > y &&
             locations(2,f) <= y+step(1) )
          feats_in_block = [feats_in_block, feats(f)];
        end
      end
      bincounts = histc(feats_in_block, (1:vocab_size));
      features = horzcat(features, bincounts ./ sum(bincounts));
      y = y+step(1);
    end
    x = x+step(2);
  end
